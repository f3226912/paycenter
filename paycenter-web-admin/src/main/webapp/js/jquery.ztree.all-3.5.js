	...
	view = {
		...
		appendNodes: function(setting, level, nodes, parentNode, index, initFlag, openFlag) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				var childHtml = [];
				if (node&&node[childKey] && node[childKey].length > 0) {
		...
		setFirstNode: function(setting, parentNode) {
			var childKey = setting.data.key.children, childLength = parentNode[childKey]?parentNode[childKey].length:0;
			if ( childLength > 0) {
				if(parentNode[childKey][0]!=undefined){
		...
		setLastNode: function(setting, parentNode) {
			var childKey = setting.data.key.children, childLength = parentNode[childKey]?parentNode[childKey].length:0;
			if ( childLength > 0) {
				if(parentNode[childKey][childLength - 1]!=undefined){
		...
		removeNode: function(setting, node) {
			...
			childLength = (function(){
				var len = 0;
				for(var i=0;i<parentNode[childKey].length;i++){
					if(parentNode[childKey][i]==undefined){
						len+=0;
					}else{
						len+=1;
					}
