 ...
 */
@Controller
@RequestMapping("gxrcb")
public class GuangxiBankNotifyController {
	...
	public void orderList(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		logger.info("广西农信获取订单列表入参:reqdata="+reqdata+",signmsg="+signmsg);
		//验签
		String appKey = "nsy_pay";
		String privateKey = null;
		ResponseDto res = new ResponseDto();
		try {
			ThirdPayConfigEntity config = thirdPayConfigService.queryByTypeSub(appKey, PayWayEnum.GXRCB.getWay(), "");
			privateKey = config.getPrivateKey();
			if(isSign && !RsaUtil.verify(MD5.crypt(reqdata).toLowerCase(), signmsg, config.getSysPublicKey(), "utf-8")){
				//验签失败
				logger.error("广西农信获取订单列表,验签失败");
			...
			logger.info("广西农信获取订单列表,返回结果:"+resMsg);
			response.getWriter().write(resMsg);
		} catch (Exception e) {
			logger.error("广西农信获取订单列表失败,"+e.getMessage(),e);
			//返回失败 
	...
	public void payNotify(HttpServletRequest request,HttpServletResponse response) throws IOException{
		String reqdata = request.getParameter("reqdata");
		String signmsg = request.getParameter("signmsg");
		logger.info("广西农信支付通知回调入参:reqdata="+reqdata+",signmsg="+signmsg);
		//验签
		String appKey = "nsy_pay";
		String privateKey = null;
		try {
			...
			if(isSign && !RsaUtil.verify(MD5.crypt(reqdata).toLowerCase(), signmsg, config.getSysPublicKey(), "utf-8")){
				//验签失败
				logger.error("广西农信支付通知,验签失败");
				response.getWriter().write(ResponseUtil.getFailSignResData(new ResponseDto(),"验签失败", privateKey));
				return;
			}
			GuangxiPayNotifyDto dto = JSON.parseObject(reqdata, GuangxiPayNotifyDto.class);
			//记录日志 以银行流水号，为基准记录日志
			PayLogRecordEntity log = PayLogUtil.buildPayReceiveLog(PayWayEnum.GXRCB.getWay(),dto.getTransseqno(), "收到广西玉林POS支付通知: "+JSON.toJSONString(dto));
			payLogRecordService.addLog(log);
			if(StringUtils.equals(dto.getPayresultcode(), "0000")){
				validate(dto);
				PosPayNotifyDto newDto = PosPayUtil.build4Guangxi(dto);
				newDto.setAppKey(appKey);
				newDto.setPayChannelCode(PayWayEnum.GXRCB.getWay());
				posPayService.payNotify(newDto);
			} else {
				logger.info("广西农信支付失败,"+dto.getPayresultmsg());
			}
